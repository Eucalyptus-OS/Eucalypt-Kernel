...
void check_function(uint8_t bus, uint8_t device, uint8_t function, uint8_t class_code, uint8_t subclass) {
    ...
    if (class_code == PCI_CLASS_STORAGE && subclass == PCI_SUBCLASS_AHCI) {
        print("Found AHCI controller on bus %d, device %d, function %d\n", bus, device, function);
    }
    // IDE controller
    if (class_code == PCI_CLASS_STORAGE && subclass == PCI_SUBCLASS_IDE) {
        print("Found IDE controller on bus %d, device %d, function %d\n", bus, device, function);
    }
    // Network controller
    if (class_code == PCI_CLASS_NETWORK && subclass == PCI_SUBCLASS_ETHERNET) {
        print("Found network controller on bus %d, device %d, function %d\n", bus, device, function);
    }
    // VGA compatible display controller
    if (class_code == PCI_CLASS_DISPLAY && subclass == PCI_SUBCLASS_VGA) {
        print("Found VGA controller on bus %d, device %d, function %d\n", bus, device, function);
    }
    // USB controller
    if (class_code == PCI_CLASS_SERIAL && subclass == PCI_SUBCLASS_USB) {
        print("Found USB controller on bus %d, device %d, function %d\n", bus, device, function);
    }
    ...
    if (class_code == PCI_CLASS_BRIDGE && subclass == PCI_SUBCLASS_PCI_PCI) {
        ...
        print("Found PCI bridge on bus %d, device %d, scanning secondary bus %d\n", bus, device, secondary_bus);
...
static void pci_log_function_id(uint8_t bus, uint8_t device, uint8_t function) {
    ...
    print("PCI: %02x:%02x.%u VID %04x PID %04x\n",
...
void pci_log_ids_once() {
    ...
    print("PCI: VID/PID enumeration begin\n");
    ...
    print("PCI: VID/PID enumeration end\n");
}