...
#[used] #[unsafe(no_mangle)] #[unsafe(link_section = ".requests")]
...
#[unsafe(no_mangle)]
extern "C" fn kmain() -> ! {
    // initialise the framebuffer renderer as early as possible for debug output
    let framebuffer_response = FRAMEBUFFER_REQUEST.response().expect("No framebuffer response");
    let framebuffer = framebuffer_response.framebuffers().first().copied().expect("No framebuffer");
    ...
    // enable SSE so userland C code can use floating point
    unsafe {
    ...
    // set up physical memory, virtual memory, and the heap allocator
    let memmap_response = MEMMAP_REQUEST.response().expect("No memory map");
    hhdm_init();
    ...
    // map the full MMIO region and then the APIC and IOAPIC specifically
    ...
    // enable interrupts and calibrate the APIC timer for preemption
    enable_apic();
    ...
    // initialise all storage controllers and enumerate PCI
    ide_init(0, 0, 0, 0, 0);
    check_all_buses();
    init_usb();
    init_ahci();
    ...
    if get_drive_count() > 0 {
        ...
        if ok { println!("  first 16 bytes: {:02x?}", &buf[..16]); }
    }

    let mp_response = MP_REQUEST.response().expect("No MP response");
    init_mp(mp_response);

    // mount the ramdisk at A: and physical drives starting at B:
    vfs_init();
    if let Some(module_response) = MODULE_REQUEST.response() {
        mount_ramdisk(module_response, 'A').expect("Failed to mount ramdisk");
    } else {
        vfs_mount('A', Box::new(ramfs::RamFs::new())).expect("Failed to mount empty ramfs");
    }

    // debug: list what the ramdisk contains
    match vfs_readdir("A:.") {
        Ok(entries) => {
            println!("A: contains {} entries:", entries.len());
            for e in &entries { println!("  '{}'", e.name); }
        }
        Err(e) => println!("A: readdir failed: {}", e),
    }

    tty::tty_init();
    tty::tty_write_str("eucalyptOS\n\n> ");

    // load the init process elf from the ramdisk
    let fd = fd_open("A:USER", O_RDONLY, 0).expect("Failed to open A:USER");
    let (entry, pml4_phys) = load_elf(fd).expect("Failed to load USER");
    ...
    process::proc::with_process_mut(init_pid, |pcb| { pcb.cr3 = pml4_phys; });

    let init_tid = TCB::create_thread(0x800000, entry, init_pid, pml4_phys).expect("Failed to create init thread");

    {
        let tcb_ptr = process::thread::get_tcb_by_tid(init_tid).expect("init TCB missing");
        process::scheduler::set_current_thread(tcb_ptr);
    }

    // create the idle process that halts when nothing else is runnable
    let idle_pid = new_process(None).expect("Failed to create idle process");
    let idle_cr3 = VMM::get_page_table() as u64;
    TCB::create_thread(0x4000, idle as *const () as u64, idle_pid, idle_cr3).expect("Failed to create idle thread");
    enable_scheduler();

    // switch to the user page table and jump to ring 3
    unsafe {
...
// spin halting forever, runs when no other thread is scheduled
fn idle() -> ! {
