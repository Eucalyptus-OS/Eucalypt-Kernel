...
pub fn load_elf(fd: u32) -> Option<(u64, u64)> {
    let mut contents = alloc::vec![0u8; 65536];
    let n = fd_read(fd, &mut contents).ok()?;
    contents.truncate(n);
    ...
    let elf = Elf64::from_bytes(&contents).ok()?;
    let ehdr = elf.ehdr();
    ...
    for ph in elf.phdr_iter() {
        ...
        if offset + filesz > contents.len() { return None; }

        let mut flags = PageTableEntry::USER | PageTableEntry::PRESENT;
        if ph.p_flags & 0x2 != 0 { flags |= PageTableEntry::WRITABLE; }
        if ph.p_flags & 0x1 == 0 { flags |= PageTableEntry::NO_EXECUTE; }
