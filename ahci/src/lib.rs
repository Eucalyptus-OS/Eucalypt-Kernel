...
use pci::{
    PCI_CLASS_MASS_STORAGE, PCI_SUBCLASS_SATA, pci_config_read_dword, pci_enable_bus_master,
    pci_enable_memory_space, pci_find_ahci_controller,
};
...
fn rebase_port(port: &mut HbaPort, portno: u32) {
    ...
    port.clb = clb_phys;
    unsafe {
        core::ptr::write_bytes(clb_virt as *mut u8, 0, 1024);
    }

    port.fb = fb_phys;
    unsafe {
        core::ptr::write_bytes(fb_virt as *mut u8, 0, 256);
    }
