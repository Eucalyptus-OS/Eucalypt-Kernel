...
impl VMM {
    ...
    pub unsafe fn map_range(virt_start: VirtAddr, phys_start: PhysAddr, flags: u64) -> Option<()> {
        unsafe {
            ...
            for _ in 0..4096 {
                ...
                current_virt = VirtAddr::new(current_virt.as_u64() + 0x1000);
                current_phys = PhysAddr::new(current_phys.as_u64() + 0x1000);
            }
            Some(())
        }
    }
